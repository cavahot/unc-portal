...
import {
  ...
  ALL_CATEGORIES,
  type CategoryGroup,
  type Ley5189Category,
} from '@/lib/cms/queries/ley5189'
...
export async function generateMetadata({
  ...
  const t = await getT(locale, 'pages.ley-5189')
  return {
    title: t('meta.title'),
      ...
      <div className="mb-8">
        ...
        <div className="mb-3 flex items-center gap-2.5">
          ...
          <h2 className="text-base font-bold tracking-tight text-[#09231D]">
            {CATEGORY_LABELS[group.category as Ley5189Category]}
          </h2>
...
export default async function Ley5189Page({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  ...
  try {
    groups = await getLey5189Documents()
  } catch {
    cmsAvailable = false
    // Show all categories empty when CMS is unavailable
    ...
    <>
      ...
      <section className="relative z-10 -mt-8 bg-[#F4F7F5] pb-20 pt-12 sm:pb-24">
        <div className="mx-auto max-w-[1260px] px-5 sm:px-6 lg:px-8">
          ...
          {/* Category groups — always render all 11 categories */}
          {groups.map((group) => (
            <CategorySection
              key={group.category}
              group={group}
